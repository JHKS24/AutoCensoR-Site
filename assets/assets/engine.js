...
(function () {
    ...
    $$("[data-ac-modelload]").forEach(function (root) {
      ...
      function set(btn) {
        ...
        if (status && btn.getAttribute("data-status")) {
          ...
          status.setAttribute("data-state", btn.getAttribute("data-mode"));
        }
        btns.forEach(function (b) {
          var on = b === btn;
          b.setAttribute("aria-pressed", on ? "true" : "false");
      ...
      var start = btns.filter(function (b) { return b.getAttribute("data-mode") === "before"; })[0] || btns[0];
      if (start) set(start);
    });
  }
  ...
  function initLightbox() {
    ...
    shots.forEach(function (shot) {
      var im = $("img", shot);
      if (!im) return;
      shot.addEventListener("click", function () { open(im.currentSrc || im.src, im.alt); });
    });
